...
def main(args):
    ...
    if isinstance(args.val_path, str):
        with open(os.path.join(args.val_path, "val.json"), "rb") as f:
            val_hash = hashlib.sha256(f.read()).hexdigest()

        val_set = RecognitionDataset(
            img_folder=os.path.join(args.val_path),
            labels_path=os.path.join(args.val_path, "val.json"),
        ...
        if len(parts) > 1:
            for subfolder in parts[1:]:
                train_set.merge_dataset(
                    RecognitionDataset(subfolder.joinpath(""), subfolder.joinpath("train.json"))
                )
    else:
        train_hash = None
        # Load synthetic data generator
